...
*/
        ...
        BlockshelfPlayground.protocolManager.addPacketListener(
                new PacketAdapter(BlockshelfPlayground.plugin, ListenerPriority.NORMAL,PacketType.Play.Client.USE_ITEM) {
                    @Override
                    public void onPacketReceiving(PacketEvent event) {
                        if (event.getPacketType() == PacketType.Play.Client.USE_ITEM) printAllEventProperties(event);
                    }});
        BlockshelfPlayground.protocolManager.addPacketListener(
                new PacketAdapter(BlockshelfPlayground.plugin, ListenerPriority.NORMAL,PacketType.Play.Client.RECIPE_DISPLAYED) {
                    @Override
                    public void onPacketReceiving(PacketEvent event) {
    ...
    public static void printAllEventProperties(PacketEvent event){
        ...
        BlockshelfPlayground.logger.warning("EVT "+event.getPacketType());
    ...
    static <T> void lazyTry(PacketContainer pack,Function<PacketContainer, StructureModifier<T>> func, String name){
        try{
            func.apply(pack).getValues().stream().forEach(
                    x-> BlockshelfPlayground.logger.warning("    "+name+"() "+atMaxLength(x.toString())));
        } catch (Exception e){
            BlockshelfPlayground.logger.severe("ERROR For:"+name);
        }
    }
